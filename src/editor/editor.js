...
window.onload = () => {
  ...
  canvasBox.style.width = `${canvasSizeWidth > 1000 ? 1000 : canvasSizeWidth}px`
  ...
  canvas_size_button.addEventListener("click", () => {
    ...
    if (!(canvas_width.value && canvas_height.value)) {
      ...
      window.alert("가능한 크기를 초과하였습니다. (최대크기: 너비1000 높이600)")
      canvas_width.value = ""
      canvas_height.value = ""
    } else {
  ...
  function seperateEffect() {
    ...
    if (!targeted) {
      ...
      targeted.filters[5] ? (vintage.checked = true) : (vintage.checked = false)
